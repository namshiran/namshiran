...
export default function ProductDetailPage() {
  ...
  const fetchProductDetails = useCallback(async () => {
    ...
    try {
      ...
      // The Noon API returns the product in data.product
      if (!data.product) {
        setError('محصول یافت نشد');
        return;
      }
  ...
  // Get the selected variant's first offer for price and availability
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        {product.variants && product.variants.length > 0 && (
          <div className="mt-8 bg-white rounded-lg shadow-lg p-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">سایزها و موجودی</h2>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {product.variants.map((variant, index) => {
                    ...
                    const variantPrice = offer?.price ?? 0;
                    const variantSalePrice = offer?.sale_price ?? null;
                    const variantStock = offer?.stock ?? 0;
                    const variantIsBuyable = offer?.is_buyable ?? false;
                    const variantDiscount =
                      variantSalePrice && variantPrice && variantSalePrice < variantPrice
                        ? Math.round((1 - variantSalePrice / variantPrice) * 100)
                        : 0;
                    
                    return (
                      <tr 
                        key={variant.sku}
                        className={`${selectedVariantIndex === index ? 'bg-blue-50' : ''} hover:bg-gray-50 transition-colors`}
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">{variant.variant}</div>
                          <div className="text-xs text-gray-500">{variant.sku}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {offer ? (
                            <div
                              className={`text-sm ${variantSalePrice ? 'line-through text-gray-500' : 'text-gray-900 font-medium'}`}
                            >
                              {formatPrice(variantPrice)} درهم
                            </div>
                          ) : (
                          ...
                          {variantSalePrice ? (
                            <div>
                              <div className="text-sm font-medium text-green-600">
                                {formatPrice(variantSalePrice)} درهم
                              </div>
                              {variantDiscount > 0 && (
                                <div className="text-xs text-red-600">
                                  {variantDiscount}% تخفیف
                                </div>
                              )}
                            </div>
                          ) : (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                            {variantStock > 0 ? `${formatPrice(variantStock)} عدد` : '۰'}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {variantStock > 0 ? (
